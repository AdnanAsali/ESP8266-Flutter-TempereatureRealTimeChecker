...
#define FIREBASE_HOST "esp8266-flutter-datapass.firebaseio.com" //Without http:// or https:// schemes
#define FIREBASE_AUTH "SjXM29uh3CbTtWbylmCfBG3vmrqkQkVCvdIEa053"
#define WIFI_SSID "Adnan"
#define WIFI_PASSWORD "adnan123"
...
//    if (Firebase.setDouble(firebaseData, path + "/Temperature/Data" + (i + 1), ((i + 1) * 10) + 0.123456789))
//    {
...
void loop()
{
   dbTempC = getSensorTempC();
   dbTempF = getSensorTempF();

   updateTempOnDB(dbTempC);
  delay(5000);
}

void updateTempOnDB(double data)
{
    String path = "/ESP8266-Temp";
  ...
  {
    //Also can use Firebase.set instead of Firebase.setDouble
    if (Firebase.setDouble(firebaseData, path + "/Temperature/Data" + (i + 1), data))
    {
      Serial.println("PASSED");
      Serial.println("PATH: " + firebaseData.dataPath());
      Serial.println("TYPE: " + firebaseData.dataType());
...
double getSensorTempC()
{
  sensors.requestTemperatures(); // Send the command to get temperatures
  return sensors.getTempCByIndex(0);
}

double getSensorTempF()
{
  sensors.requestTemperatures(); // Send the command to get temperatures
